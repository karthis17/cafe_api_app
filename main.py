...
# Connect to Database
app.config[
    'SQLALCHEMY_DATABASE_URI'] = "postgresql://admin:htmH2VfF2UD7fEdDpItyv5tmRougniRQ@dpg-ch40h7seooglumagtc4g-a.oregon-postgres.render.com/blog_ftc6"
...
def get_cafe_at_location():
    ...
    cafes = db.session.query(Cafe).filter(Cafe.location.ilike(f'%{query_location}%')).all()
    if cafes:
        return jsonify(cafe={cafe.id: cafe.to_dict() for cafe in cafes})
    else:
        return jsonify(error={"Not Found": "Sorry, we don't know any cafe at that location."})
...
def add_cafe():
    ...
    try:
        ...
        db.session.commit()
        return jsonify(Success="New cafe details are successfully added! ")
    except:
        return jsonify(Failed="Adding process not well done please check the data")
...
@app.route("/delete-report/<_id_>", methods=["POST", "GET"])
def report_to_delete(_id_):
    cafe = db.session.query(Cafe).get(_id_)
    api_secret_key = "qwertyuiopasdfghjkl"
    if cafe:
